...
for idx, measurement in enumerate(measurements["measurements"]):
  row_val = idx+2
  ws.cell(row=idx+3, column=1, value=measurement)
  ws.cell(row=idx+3, column=2, value=(measurement + measurements["distance"]))
...
iri = Reference(ws, min_col=3, max_col=3, min_row=3, max_row=length)
filter_data = Reference(ws, min_col=4, max_col=4, min_row=3, max_row=length)
segmentation_data = Reference(ws, min_col=5, max_col=5, min_row=3, max_row=length)
